...
| Function FQCN rename | Function declarations and direct namespace parent | Function calls returned by `member-graph` | Normal and grouped `use function` imports, explicit alias preservation, standalone import fallback for incompatible grouped prefixes, and fully-qualified fallback on alias conflict | Structured function `@see` references, including FQCN references | Target namespace function declarations and function import aliases | Does not move physical file path |
...
| Namespace-level constant FQCN rename | Namespace-level `Const_` declarations and direct namespace parent | Constant fetches returned by `member-graph` | Normal and grouped `use const` imports, explicit alias preservation, standalone import fallback for incompatible grouped prefixes, and fully-qualified fallback on alias conflict | Structured constant `@see` references, including FQCN references | Target namespace constant declarations and constant import aliases | Does not move physical file path |
...
| Nested callable local variable rename | Not applicable | Local variable usages inside the selected callable body, explicit closure captures, implicit arrow-function captures, and selected dynamic-variable expressions | Not applicable | Not supported | Same-callable parameters, local variables, and PHP superglobals | Conservative scoped rename only; shadowed nested callable parameters are skipped |
