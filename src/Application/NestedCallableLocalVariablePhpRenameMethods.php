...
trait NestedCallableLocalVariablePhpRenameMethods
{
    ...
    /**
     * Plans a closure local variable rename inside a method container.
     *
     * @param string               $className      the method owner FQCN
     * @param string               $methodName     the method name
     * @param int                  $closureIndex   the zero-based closure index inside the method
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function planClosureLocalVariableRenameInMethod(
        string $className,
        string $methodName,
        int $closureIndex,
        string $variableName,
        ...
        return $this->planNestedCallableLocalVariableRename($this->methodNestedCallableLocalVariableRequest(
            NestedCallableKind::CLOSURE,
            $className,
            $methodName,
            $closureIndex,
    ...
    public function renameClosureLocalVariableInMethod(
        ...
        return $this->renamePlanApplier->apply(
            plan: $this->planClosureLocalVariableRenameInMethod($className, $methodName, $closureIndex, $variableName, $newName, $conflictPolicy),
            build: $this->build,
        );
    }

    /**
     * Executes one arrow-function local variable rename step inside a method container.
     *
     * @param RenameStepContext    $context        the current rename step context
     * @param string               $className      the method owner FQCN
     * @param string               $methodName     the method name
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function executeStepArrowFunctionLocalVariableRenameInMethod(
        RenameStepContext $context,
        string $className,
        string $methodName,
        int $arrowIndex,
    ...
    /**
     * Plans an arrow-function local variable rename inside a method container.
     *
     * @param string               $className      the method owner FQCN
     * @param string               $methodName     the method name
     * @param int                  $arrowIndex     the zero-based arrow-function index inside the method
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function planArrowFunctionLocalVariableRenameInMethod(
        string $className,
        string $methodName,
        int $arrowIndex,
        string $variableName,
    ...
    public function renameArrowFunctionLocalVariableInMethod(
        ...
        return $this->renamePlanApplier->apply(
            plan: $this->planArrowFunctionLocalVariableRenameInMethod($className, $methodName, $arrowIndex, $variableName, $newName, $conflictPolicy),
            build: $this->build,
        );
    }

    /**
     * Executes one closure local variable rename step inside a function container.
     *
     * @param RenameStepContext    $context        the current rename step context
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function executeStepClosureLocalVariableRenameInFunction(
        RenameStepContext $context,
        string $functionName,
        int $closureIndex,
        string $variableName,
    ...
    /**
     * Plans a closure local variable rename inside a function container.
     *
     * @param string               $functionName   the fully-qualified function name
     * @param int                  $closureIndex   the zero-based closure index inside the function
     * @param string               $variableName   the current variable name without "$"
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function planClosureLocalVariableRenameInFunction(
        string $functionName,
        int $closureIndex,
        string $variableName,
        string $newName,
        ...
        return $this->planNestedCallableLocalVariableRename($this->functionNestedCallableLocalVariableRequest(
            NestedCallableKind::CLOSURE,
            $functionName,
            $closureIndex,
            $variableName,
    ...
    public function renameClosureLocalVariableInFunction(
        ...
        return $this->renamePlanApplier->apply(
            plan: $this->planClosureLocalVariableRenameInFunction($functionName, $closureIndex, $variableName, $newName, $conflictPolicy),
            build: $this->build,
        );
    }
    ...
    /**
     * Plans an arrow-function local variable rename inside a function container.
     *
     * @param string               $functionName   the fully-qualified function name
     * @param int                  $arrowIndex     the zero-based arrow-function index inside the function
     * @param string               $variableName   the current variable name without "$"
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function planArrowFunctionLocalVariableRenameInFunction(
        string $functionName,
        int $arrowIndex,
        string $variableName,
        string $newName,
    ...
    public function renameArrowFunctionLocalVariableInFunction(
        ...
        return $this->renamePlanApplier->apply(
            plan: $this->planArrowFunctionLocalVariableRenameInFunction($functionName, $arrowIndex, $variableName, $newName, $conflictPolicy),
            build: $this->build,
        );
    }
    ...
    /**
     * Plans a closure local variable rename inside a file container.
     *
     * @param string               $filePath       the physical or virtual file path
     * @param int                  $closureIndex   the zero-based closure index inside the file
     * @param string               $variableName   the current variable name without "$"
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function planClosureLocalVariableRenameInFile(
        string $filePath,
        int $closureIndex,
        string $variableName,
        string $newName,
        ...
        return $this->planNestedCallableLocalVariableRename($this->fileNestedCallableLocalVariableRequest(
            NestedCallableKind::CLOSURE,
            $filePath,
            $closureIndex,
            $variableName,
    ...
    public function renameClosureLocalVariableInFile(
        ...
        return $this->renamePlanApplier->apply(
            plan: $this->planClosureLocalVariableRenameInFile($filePath, $closureIndex, $variableName, $newName, $conflictPolicy),
            build: $this->build,
        );
    }

    /**
     * Executes one arrow-function local variable rename step inside a file container.
     *
     * @param RenameStepContext    $context        the current rename step context
     * @param string               $filePath       the physical or virtual file path
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function executeStepArrowFunctionLocalVariableRenameInFile(
        RenameStepContext $context,
        string $filePath,
        int $arrowIndex,
        string $variableName,
    ...
    /**
     * Plans an arrow-function local variable rename inside a file container.
     *
     * @param string               $filePath       the physical or virtual file path
     * @param int                  $arrowIndex     the zero-based arrow-function index inside the file
     * @param string               $variableName   the current variable name without "$"
     ...
     * @throws \InvalidArgumentException when one rename input is invalid
     */
    public function planArrowFunctionLocalVariableRenameInFile(
        string $filePath,
        int $arrowIndex,
        string $variableName,
        string $newName,
    ...
    public function renameArrowFunctionLocalVariableInFile(
        ...
        return $this->renamePlanApplier->apply(
            plan: $this->planArrowFunctionLocalVariableRenameInFile($filePath, $arrowIndex, $variableName, $newName, $conflictPolicy),
            build: $this->build,
        );
    }
